...
void ImageDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    for( int j = 0; j < lines_[0].second.size(); j++){
/*        if( image_data_param.transform_ann()){
    		Dtype tmp = lines_[lines_id_].second[j];
    		...
    		top_label[item_id * lines_[0].second.size() + j ] = tmp;
        } else {*/
        	top_label[item_id * lines_[0].second.size() + j ] = lines_[lines_id_].second[j];
//        }
    }
...
void ImageDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  prefetch_label_.Reshape( batch_size, lines_[0].second.size(), 1, 1);
